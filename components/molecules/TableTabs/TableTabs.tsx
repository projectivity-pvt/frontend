...
export default function TableTabs () {
 ...
 <div className="tabs">
  <div onClick={()=>action(1)} className={`${tab === 1 ? 'tab active_tab' : 'tab' }`}>Tab 1</div>
  <div onClick={()=>action(2)} className={`${tab === 2 ? 'tab active_tab' : 'tab' }`}>Tab 2</div>
  <div onClick={()=>action(3)} className={`${tab === 3 ? 'tab active_tab' : 'tab' }`}>Tab 3</div>
 </div>
